        ...
        <v-card-text>
          ...
          >
            <v-row>
              <v-col cols="4">
                <v-autocomplete
                  v-model="price.request.branchId"
                  :items="branches"
                  item-value="id"
              ...
              ><v-col cols="4">
                <v-autocomplete
                  v-model="price.request.productId"
                  :items="products"
                  item-value="id"
          ...
          <v-card elevation="0" variant="outlined" class="pa-5">
            <v-card variant="outlined" elevation="0" class="mb-5">
              <div class="d-flex align-center justify-center ml-4">
                <h3>{{ $t("select_main_currency") }}:</h3>
                <v-radio-group
                  v-model="price.request.selectCurrency"
                  inline
                  class="ml-4 mt-5"
                >
            ...
            <v-row>
              <v-col cols="3">
                <span>
                  <p>{{ $t("price_unit") }} <b class="text-blue">LAK</b></p>
                </span>
                <v-text-field
                  rounded="xl"
                  clearable
                  v-model="price.request.lakUnit"
                  @input="
                    price.request.lakUnit = formatCurrency(
                      price.request.lakUnit,
                    )
                  "
                ></v-text-field
              ></v-col>
              <v-col cols="3">
                <span>
                  <p>{{ $t("price_package") }} <b class="text-blue">LAK</b></p>
                </span>
                <v-text-field
                  rounded="xl"
                  clearable
                  v-model="price.request.lakPackage"
                  @input="
                    price.request.lakPackage = formatCurrency(
                      price.request.lakPackage,
                    )
                  "
                ></v-text-field
              ></v-col>
              <v-col cols="3">
                <span>
                  <p>{{ $t("price_unit") }} <b class="text-red">THB</b></p>
                </span>
                <v-text-field
                  rounded="xl"
                  clearable
                  v-model="price.request.thbUnit"
                  @input="
                    price.request.thbUnit = formatCurrency(
                      price.request.thbUnit,
                    )
                  "
                ></v-text-field
              ></v-col>
              <v-col cols="3">
                <span>
                  <p>{{ $t("price_package") }} <b class="text-red">THB</b></p>
                </span>
                <v-text-field
                  rounded="xl"
                  clearable
                  v-model="price.request.thbPackage"
                  @input="
                    price.request.thbPackage = formatCurrency(
                      price.request.thbPackage,
                    )
                  "
                ></v-text-field
              ></v-col>
              <v-col cols="3">
                <span>
                  <p>{{ $t("price_unit") }} <b class="text-green">USD</b></p>
                </span>
                <v-text-field
                  rounded="xl"
                  clearable
                  v-model="price.request.usdUnit"
                  @input="
                    price.request.usdUnit = formatCurrency(
                      price.request.usdUnit,
                    )
                  "
                ></v-text-field
              ></v-col>
              <v-col cols="3">
                <span>
                  <p>{{ $t("price_package") }} <b class="text-green">USD</b></p>
                </span>
                <v-text-field
                  rounded="xl"
                  clearable
                  v-model="price.request.usdPackage"
                  @input="
                    price.request.usdPackage = formatCurrency(
                      price.request.usdPackage,
                    )
                  "
                ></v-text-field></v-col
            ></v-row>
          ...
          <v-row justify="start"
            ><v-col cols="12" md="6" sm="6">
              <v-btn
                ...
                rounded="xl"
                variant="outlined"
                @click="price.clearData()"
                ><v-icon class="mr-4">mdi-cancel</v-icon
                >{{ $t("btn_cancel") }}</v-btn
              >
              <v-btn
                v-if="price.request.edit == false"
                color="primary"
                rounded="xl"
                variant="outlined"
                type="submit"
...
const insertPrict = async () => {
  if (!price.request.branchId && !price.request.productId) {
    ...
    if (
      price.request.lakPackage == null &&
      price.request.lakUnit == null &&
      price.request.thbPackage == null &&
      price.request.thbUnit == null &&
      price.request.usdPackage == null &&
      price.request.usdUnit == null
    ) {
